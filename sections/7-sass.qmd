...
::: {.body-text}
Sass allows us to **define variables** (in the form `$var-name: value;`) for our colors to reference instead of writing out their HEX codes each time. This makes your stylesheet **more readable and easier to update** (only need to change HEX codes in one spot, not multiple!)
:::
...
::: {.center-text}
From here on out, we'll be writing our Sass variables and CSS rules in a `.scss` file, which we'll use to style our a fun/silly sample report.
:::
